...
const uploadTempShortcut = async (req, res) => {
    try {
        ...
        if (!req.file) {
            return res.status(400).json({
                success: false,
                message: 'No shortcut file uploaded'
            });
        }
        ...
        res.status(200).json({
            success: true,
            shortcutUrl: publicShortcutUrl,
            message: 'Shortcut file uploaded successfully'
        });

        // Set up cleanup of the temporary file after 5 minutes
        setTimeout(() => {
        ...
        console.error('Error uploading temporary shortcut:', error);
        res.status(500).json({
            success: false,
            message: 'Internal server error: ' + error.message
        });
